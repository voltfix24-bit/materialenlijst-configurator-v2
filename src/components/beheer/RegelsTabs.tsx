...
const RMU_MERKEN = ["", "Magnefix", "ABB", "Siemens"];
const RMU_VT = ["", "F", "C", "V"];
const RMU_KT = ["", "240AL", "630AL"];
const RMU_KVA = ["", "250", "400", "630"];
const RMU_KL = ["", "7.25", "10"];
const RMU_SECTIE = ["rmu", "trafo"];

export function RmuVeldRegelsTab() {
  const qc = useQueryClient();
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Partial<RmuVeldRegel> | null>(null);
  const [toDelete, setToDelete] = useState<RmuVeldRegel | null>(null);

  const { data = [] } = useQuery({
    queryKey: ["beheer-rmuveld-regels"],
    queryFn: async () => (await supabase.from("rmu_veld_regels").select("*").order("sort_order")).data ?? [],
  });

  const save = useMutation({
    mutationFn: async (v: Partial<RmuVeldRegel>) => {
      const payload: any = { ...v };
      for (const k of ["conditie_merk", "conditie_veld_type", "conditie_kabel_type", "conditie_kva", "conditie_trafo_kabel_lengte"] as const) {
        if (payload[k] === "") payload[k] = null;
      }
      if (v.id) {
        const { error } = await supabase.from("rmu_veld_regels").update(payload).eq("id", v.id);
        if (error) throw error;
      } else {
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["beheer-rmuveld-regels"] }); qc.invalidateQueries({ queryKey: ["rmu_veld_regels"] }); toast.success("Opgeslagen"); setOpen(false); },
    onError: (e: any) => toast.error(e.message),
  });

  const del = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("rmu_veld_regels").delete().eq("id", id); if (error) throw error; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["beheer-rmuveld-regels"] }); qc.invalidateQueries({ queryKey: ["rmu_veld_regels"] }); toast.success("Verwijderd"); setToDelete(null); },
  });

  const startNew = () => { setEditing({ hoeveelheid: 1, sort_order: (data as RmuVeldRegel[]).length, actief: true, herkomst_label: "RMU", sectie: "rmu" }); setOpen(true); };
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <Button onClick={startNew}><Plus className="h-4 w-4 mr-1" /> RMU veld regel toevoegen</Button>
      </div>
      <p className="text-xs text-muted-foreground">
        Per veld in de RMU-config wordt elke matching regel toegevoegd. Label-placeholder <code>{`{veldNummer}`}</code> wordt vervangen door het veldnummer.
        Conditie <code>aantal_kv_min/max</code> telt totaal aantal C+V velden (voor Magnefix doos-keuze).
      </p>
      <DataTable
        headers={["Merk", "I-Net", "VT", "V#=1", "Res", "Kabel", "kVA", "TrafoKab", "KV min/max", "Artikel", "Aantal", "Sectie", "Herkomst", ""]}
        rows={(data as RmuVeldRegel[]).map((r) => [
          <Cond v={r.conditie_merk} />,
          <Cond v={r.conditie_is_inet} />,
          <Cond v={r.conditie_veld_type} />,
          <Cond v={r.conditie_veld_nummer_is_1} />,
          <Cond v={r.conditie_is_reserve} />,
          <Cond v={r.conditie_kabel_type} />,
          <Cond v={r.conditie_kva} />,
          <Cond v={r.conditie_trafo_kabel_lengte} />,
          <span className="font-mono text-xs">{r.conditie_aantal_kv_min ?? "—"}/{r.conditie_aantal_kv_max ?? "—"}</span>,
          <ArtikelLabel id={r.artikel_id} />,
          <span className="text-xs">{r.hoeveelheid}</span>,
          <span className="text-xs">{r.sectie}</span>,
          <span className="text-xs">{r.herkomst_label}</span>,
          <RowActions onEdit={() => { setEditing(r); setOpen(true); }} onDelete={() => setToDelete(r)} />,
        ])}
        emptyIcon={Wrench}
        emptyMessage="Nog geen RMU veld regels"
        emptyDescription="Conditionele regels voor RMU eindsluitingen, buispatronen, ombouwsets en trafokabel."
        emptyAction={<Button onClick={startNew}><Plus className="h-4 w-4 mr-1" /> Eerste RMU veld regel</Button>}
      />
      <FormDialog open={open} onOpenChange={setOpen} title={editing?.id ? "RMU veld regel bewerken" : "RMU veld regel toevoegen"} size="lg">
        {editing && (
          <div className="space-y-3">
            <FormRow>
              <FormField label="Merk">
                <select value={editing.conditie_merk ?? ""} onChange={(e) => setEditing({ ...editing, conditie_merk: e.target.value || null })} className="h-9 rounded-md border border-border bg-surface px-2 text-sm">
                  {RMU_MERKEN.map((x) => <option key={x} value={x}>{x || "— alle —"}</option>)}
                </select>
              </FormField>
              <FormField label="I-Net"><TriBool value={editing.conditie_is_inet ?? null} onChange={(v) => setEditing({ ...editing, conditie_is_inet: v })} /></FormField>
              <FormField label="Veld type">
                <select value={editing.conditie_veld_type ?? ""} onChange={(e) => setEditing({ ...editing, conditie_veld_type: e.target.value || null })} className="h-9 rounded-md border border-border bg-surface px-2 text-sm">
                  {RMU_VT.map((x) => <option key={x} value={x}>{x || "— alle —"}</option>)}
                </select>
              </FormField>
              <FormField label="Veldnummer = 1"><TriBool value={editing.conditie_veld_nummer_is_1 ?? null} onChange={(v) => setEditing({ ...editing, conditie_veld_nummer_is_1: v })} /></FormField>
              <FormField label="Is reserve"><TriBool value={editing.conditie_is_reserve ?? null} onChange={(v) => setEditing({ ...editing, conditie_is_reserve: v })} /></FormField>
            </FormRow>
            <FormRow>
              <FormField label="Kabeltype">
                <select value={editing.conditie_kabel_type ?? ""} onChange={(e) => setEditing({ ...editing, conditie_kabel_type: e.target.value || null })} className="h-9 rounded-md border border-border bg-surface px-2 text-sm">
                  {RMU_KT.map((x) => <option key={x} value={x}>{x || "— alle —"}</option>)}
                </select>
              </FormField>
              <FormField label="kVA">
                <select value={editing.conditie_kva ?? ""} onChange={(e) => setEditing({ ...editing, conditie_kva: e.target.value || null })} className="h-9 rounded-md border border-border bg-surface px-2 text-sm">
                  {RMU_KVA.map((x) => <option key={x} value={x}>{x || "— alle —"}</option>)}
                </select>
              </FormField>
              <FormField label="Trafo kabel">
                <select value={editing.conditie_trafo_kabel_lengte ?? ""} onChange={(e) => setEditing({ ...editing, conditie_trafo_kabel_lengte: e.target.value || null })} className="h-9 rounded-md border border-border bg-surface px-2 text-sm">
                  {RMU_KL.map((x) => <option key={x} value={x}>{x || "— alle —"}</option>)}
                </select>
              </FormField>
              <FormField label="Aantal KV min">
                <Input type="number" value={editing.conditie_aantal_kv_min ?? ""} onChange={(e) => setEditing({ ...editing, conditie_aantal_kv_min: e.target.value === "" ? null : Number(e.target.value) })} className="h-9" />
              </FormField>
              <FormField label="Aantal KV max">
                <Input type="number" value={editing.conditie_aantal_kv_max ?? ""} onChange={(e) => setEditing({ ...editing, conditie_aantal_kv_max: e.target.value === "" ? null : Number(e.target.value) })} className="h-9" />
              </FormField>
            </FormRow>
            <FormField label="Artikel" required>
              <ArtikelZoeker value={editing.artikel_id ?? null} onChange={(id) => setEditing({ ...editing, artikel_id: id ?? undefined })} categorieSuggesties={["RMU", "Trafo"]} />
            </FormField>
            <FormRow>
              <FormField label="Hoeveelheid">
                <Input type="number" value={editing.hoeveelheid ?? 1} onChange={(e) => setEditing({ ...editing, hoeveelheid: Number(e.target.value) })} className="h-9" />
              </FormField>
              <FormField label="Sectie">
                <select value={editing.sectie ?? "rmu"} onChange={(e) => setEditing({ ...editing, sectie: e.target.value })} className="h-9 rounded-md border border-border bg-surface px-2 text-sm">
                  {RMU_SECTIE.map((x) => <option key={x} value={x}>{x}</option>)}
                </select>
              </FormField>
              <FormField label="Volgorde">
                <Input type="number" value={editing.sort_order ?? 0} onChange={(e) => setEditing({ ...editing, sort_order: Number(e.target.value) })} className="h-9" />
            ...
            <FormField label="Herkomst label (mag {veldNummer} bevatten)">
              <Input value={editing.herkomst_label ?? ""} onChange={(e) => setEditing({ ...editing, herkomst_label: e.target.value })} className="h-9" />
            </FormField>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={editing.actief ?? true} onChange={(e) => setEditing({ ...editing, actief: e.target.checked })} />
